...
-- Check for NULL Primary Keys separately (better performance)
SELECT *
FROM silver.crm_cust_info
...
-- Data Standardization & Consistency (e.g., Marital Status)
SELECT DISTINCT 
    cst_marital_status 
FROM silver.crm_cust_info
ORDER BY cst_marital_status;

-- Optional: Detect invalid marital statuses
SELECT DISTINCT 
    cst_marital_status
FROM silver.crm_cust_info
...
-- Use ROUND to avoid floating-point precision mismatches
SELECT DISTINCT 
    sls_sales,
    sls_quantity,
    sls_price
...
-- Check for Unwanted Spaces in Category Columns
SELECT 
    * 
